...
function panelInfo(id) {
    d3.json(url).then(function (data) { //loading json data from the url specified above
        let sampleData = data;
        let metadata = sampleData.metadata;
        let identifier = metadata.filter(sample => //gettign metadata for a sample based on ID
            sample.id.toString() === id)[0];
        let panel = d3.select('#sample-metadata');
        panel.html('');
        Object.entries(identifier).forEach(([key, value]) => {  
            panel.append('h6').text(`${key}: ${value}`); //updating html with ID 
        })
    })
};
...
//when selecting a different option in drop down, updating info in plots and panel 
function optionChanged(id) {
    Plots(id);
    panelInfo(id);
};

//Dropdown basd upon what is selected
function init() {
